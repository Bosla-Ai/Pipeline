...
def test_live_graph_validation():
    ...
    assert len(ctx_alias_errors) == 0, f"Context aliases load failed: {ctx_alias_errors}"
...
def test_context_alias_conflicts_with_simple_alias():
    ...
    errors, warnings = validate_context_aliases(mock_ctx_aliases, mock_graph, mock_simple_aliases)
