        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            for key, val in data.items():
                if key in combined_graph:
                    raise ValueError(
                        f"Duplicate canonical node ID found: '{key}' (in both {node_sources[key]} and {path.name})"
                    )
...
def generate_inventories_payloads() -> tuple[dict, dict]:
    ...
    for alias_key, target in aliases.items():
        if target not in combined_graph:
            raise ValueError(
                f"Alias target '{target}' for alias '{alias_key}' does not exist in canonical tags"
            )
    ...
    for alias_key, candidates in context_aliases.items():
        ...
        for cand in candidates:
            ...
            if not target:
                raise ValueError(
                    f"Context alias '{alias_key}' candidate is missing 'target'"
                )
            if target not in combined_graph:
                raise ValueError(
                    f"Context alias target '{target}' for context alias '{alias_key}' does not exist in canonical tags"
                )
    ...
    for node_id, domain in domain_mappings.items():
        if node_id not in combined_graph:
            raise ValueError(
                f"Domain mapping node ID '{node_id}' does not exist in canonical tags"
            )
    ...
    for node_id, val in combined_graph.items():
        if val is None:
            ...
            if difficulty is not None and not isinstance(difficulty, str):
                raise ValueError(
                    f"Node '{node_id}' has invalid difficulty type: {type(difficulty)} (expected string)"
                )

            est_hours = val.get("estimated_hours")
            if est_hours is not None:
                if isinstance(est_hours, bool) or not isinstance(
                    est_hours, (int, float)
                ):
                    raise ValueError(
                        f"Node '{node_id}' has invalid estimated_hours type: {type(est_hours)} (expected int or float)"
                    )
        else:
            raise ValueError(
                f"Node '{node_id}' has invalid structure type: {type(val)}"
            )

        if not isinstance(prereqs, list):
            raise ValueError(
                f"Node '{node_id}' prerequisites must be a list (got {type(prereqs)})"
            )
        ...
        for prereq in prereqs:
            if not isinstance(prereq, str):
                raise ValueError(
                    f"Node '{node_id}' prerequisite '{prereq}' is not a string"
                )
            ...
            if norm_prereq not in combined_graph:
                raise ValueError(
                    f"Node '{node_id}' has missing/unknown prerequisite target '{prereq}' (normalized: '{norm_prereq}')"
                )
